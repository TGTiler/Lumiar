...
export function HomeScreen({ navigation, resetKey }: HomeScreenProps) {
  ...
  useEffect(() => {
    if (resetKey && resetKey > 0) {
      ...
      setPage(1);
      setHasMore(allApps.length > PAGE_SIZE);
  ...
  const loadMoreApps = () => {
    if (loadingMore || !hasMore || searchResults !== null || selectedCategory) return;
    setLoadingMore(true);
    setTimeout(() => {
      const nextPage = page + 1;
      const newDisplayed = allApps.slice(0, nextPage * PAGE_SIZE);
      setPage(nextPage);
      setHasMore(newDisplayed.length < allApps.length);
      setLoadingMore(false);
    }, 500);
  };

  const filteredApps = selectedCategory
    ? allApps.filter((a) => a.categoria === selectedCategory)
    : allApps.slice(0, page * PAGE_SIZE);

  const featuredApps = api.getFeaturedApps(allApps, 5);
      ...
      <View style={styles.landscapeContainer}>
        <View style={styles.landscapeContent}>
          <View style={styles.landscapeHeader}>
            ...
            <TouchableOpacity style={styles.avatarButton} onPress={() => navigation.navigate('ProfileModal')} focusable={true}>
              <AvatarIcon name={profile.name} size={36} />
            </TouchableOpacity>
          </View>
          <View style={styles.searchContainer}>
            <Ionicons name="search" size={18} color={Colors.textMuted} />
            <TextInput style={styles.searchInput} placeholder="Buscar apps..." placeholderTextColor={Colors.textMuted} value={searchQuery} onChangeText={handleSearch} />
          ...
          {!searchResults && (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsList}>
              <TouchableOpacity style={[styles.chip, !selectedCategory && styles.chipActive]} onPress={() => setSelectedCategory(null)} focusable={true}>
                <Text style={[styles.chipText, !selectedCategory && styles.chipTextActive]}>Todos</Text>
              </TouchableOpacity>
              {categories.map((cat) => (
                <TouchableOpacity key={cat.slug} style={[styles.chip, selectedCategory === cat.nome && styles.chipActive]} onPress={() => setSelectedCategory(selectedCategory === cat.nome ? null : cat.nome)} focusable={true}>
                  <Text style={styles.chipIcon}>{cat.icon}</Text>
                  <Text style={[styles.chipText, selectedCategory === cat.nome && styles.chipTextActive]}>{cat.nome}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          )}
          <FlatList
            data={searchResults || filteredApps}
            ...
            onEndReached={searchResults ? undefined : loadMoreApps}
            onEndReachedThreshold={0.5}
            ListFooterComponent={loadingMore ? <ActivityIndicator color={Colors.primary} style={{ padding: 16 }} /> : null}
            renderItem={({ item }) => (
              <TouchableOpacity key={item.ID} style={styles.landscapeGridCard} onPress={() => navigateToApp(item)} focusable={true}>
  ...
  // Portrait - Single FlatList
  const listData = searchResults !== null ? searchResults : filteredApps;
    ...
    <>
      ...
      {searchResults !== null && searchResults.length > 0 && (
        <View style={styles.section}>
          ...
          <TouchableOpacity style={styles.searchFeaturedCard} onPress={() => navigateToApp(searchResults[0])} activeOpacity={0.7} focusable={true}>
            <View style={styles.searchFeaturedTop}>
              {isValidUrl(searchResults[0].logo) ? (
                ...
                <View style={styles.searchFeaturedFallback}><Text style={styles.searchFeaturedFallbackText}>{getInitial(searchResults[0].NomeAPP)}</Text></View>
              )}
              <View style={styles.searchFeaturedInfo}>
                <Text style={styles.searchFeaturedName}>{searchResults[0].NomeAPP}</Text>
                <Text style={styles.searchFeaturedDesc} numberOfLines={2}>{searchResults[0].Descricao || searchResults[0].descricao || ''}</Text>
                <View style={styles.searchFeaturedMeta}>
                  <Text style={styles.searchFeaturedVersion}>v{searchResults[0].Versao}</Text>
                  <View style={styles.searchFeaturedBadge}><Text style={styles.searchFeaturedBadgeText}>{searchResults[0].categoria}</Text></View>
                </View>
              </View>
            </View>
          ...
          {(() => {
            ...
            return (
              <View style={styles.similarSection}>
                ...
                <FlatList data={similar} keyExtractor={(item) => item.ID} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.similarList}
                  renderItem={({ item }) => (
                    <TouchableOpacity style={styles.similarCard} onPress={() => navigateToApp(item)} activeOpacity={0.7} focusable={true}>
                      {isValidUrl(item.logo) ? <Image source={{ uri: item.logo }} style={styles.similarLogo} resizeMode="cover" /> : <View style={styles.similarFallback}><Text style={styles.similarFallbackText}>{getInitial(item.NomeAPP)}</Text></View>}
      ...
      {/* Featured Section (no search) */}
      {!searchResults && !selectedCategory && featuredApps.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Destaques</Text>
            <Text style={styles.sectionCount}>{featuredApps.length} apps</Text>
          </View>
          <FlatList data={featuredApps} keyExtractor={(item) => item.ID} horizontal showsHorizontalScrollIndicator={false}
            snapToInterval={300} snapToAlignment="center" decelerationRate="fast" contentContainerStyle={styles.featuredList}
            onViewableItemsChanged={({ viewableItems }) => { if (viewableItems.length > 0 && viewableItems[0].index != null) setFeaturedIndex(viewableItems[0].index); }}
            viewabilityConfig={{ viewAreaCoveragePercentThreshold: 50 }}
            renderItem={({ item }) => <FeaturedCard app={item} onPress={() => navigateToApp(item)} />}
          />
          {featuredApps.length > 1 && (
            <View style={styles.dotsContainer}>
              {featuredApps.map((_, i) => <View key={i} style={[styles.dot, i === featuredIndex && styles.dotActive]} />)}
            </View>
          )}
        </View>
      )}
      ...
      {searchResults === null && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>{selectedCategory ? selectedCategory : 'Todos os Apps'}</Text>
        </View>
      )}
    </>
  );
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => <AppListItem app={item} onPress={() => navigateToApp(item)} />}
...
const styles = StyleSheet.create({
  ...
  searchFeaturedCard: {
    ...
    padding: Spacing.md, borderWidth: 1, borderColor: Colors.primary + '40', marginBottom: Spacing.md,
  },
